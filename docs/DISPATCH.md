...
9. **OPEN DECISION rule:** Q1–Q7 and Q9–Q17 (SCAFFOLD §7) are open — Q8 and Q18 are resolved. A unit gated on an open one may build to the documented working assumption only where BUILD_PLAN says so (e.g., Q5), must leave a labeled seam, and must never resolve the decision in code. A decision that changes a phase's inputs re-runs that phase's gates (TEST_PLAN §5). Flag every assumption in the HANDOFF entry.
...
| D-31 | P7 could-haves | P7 tail (conditional) | E6, F5, I5 | D-23, D-24, D-26 (dispatch only if Must on weeks 9–10 is stable — Recipe_Systems §13) | ✅ below | 🟢 UNBLOCKED (§13 satisfied 2026-09-16) |
...
## D-06 — P1-1: IdP setup (Keycloak)

**Session budget:** 1 session · **Depends on:** D-05 (P0 exit) · **Audit:** A-06

```text
You are setting up the pilot identity provider. Keycloak is the sole identity/authentication provider
(Q8 RESOLVED 2026-09-07 — SCAFFOLD §7; Tech Stack §1/§14/§25/§26 name Keycloak, OIDC/OAuth2). This
unit is dispatched at P1 (weeks 3–4) — NOT before (do not start Keycloak yet).
...
2. Leave the OIDC/OAuth2 boundary abstract behind the adapter seam: app code consumes OIDC claims,
   never Keycloak-specific APIs directly — the portability seam BUILD_PLAN §5 demands.
3. No alternate IdP configuration (Auth0 or otherwise) is introduced anywhere — Q8 is resolved;
   reintroducing an alternate provider is a BLOCKER (A-06).
...
- The adapter seam is demonstrable: swapping the adapter's provider config is a config change, not code.
- HANDOFF.md records the Keycloak decision reference (Q8 resolved, SCAFFOLD §7).
```
