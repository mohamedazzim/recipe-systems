...
flowchart TB
    ...
    P0 --> P1["P1 · wks 3–4<br/>Identity: Keycloak IdP · account · guest claim"]
...
**Critical path: P0 → P1 → P2 → P3 → P4 → P5 → P6 → P7.** Track R feeds P4 and Track S feeds P5 but neither blocks the spine. P2 depends on P1's guest sessions only — IdP setup does not gate intake. The analysis core (P3) is the riskiest node: prompts and grounding must both land inside weeks 5–6, consuming the schemas frozen at P0.
...
**Not needed yet:** analysis worker, the IdP (Keycloak), UI beyond a shell.

### P1 — Identity & account (wks 3–4)

| # | Work | Source |
|---|---|---|
| P1-1 | IdP setup: Keycloak realm `recipesystems` + realm export in `infra/keycloak/` (Q8 resolved 2026-09-07; not started before P1) | BUILD_PLAN §5 |
...
## 5. IdP position — Keycloak (Q8 resolved)

| Aspect | Position |
|---|---|
| Phase | P1 — container in compose, realm export in `infra/keycloak/`. **Not started before P1; do not start Keycloak yet** (sequence constraint — the IdP is not needed by P0) |
| Decision | **Q8 RESOLVED 2026-09-07:** Keycloak is the sole identity/authentication provider (OIDC/OAuth2). Tech Stack §1/§14/§25/§26 amended to name Keycloak; ADR §19 names it; Auth0 is removed from the active architecture (historical trail in CHANGE_LOG.md) |
| Role | External trust boundary (ADR §10 `IDP` node). Owns credential lifecycle, password reset, email verification, OAuth/SSO, MFA — the Tech Stack §14 IdP list |
...
| Datastore | Dev: `keycloak` database in the compose Postgres (dev convention). Production: separate database on the managed Postgres instance (least-infra option) — **OPEN DECISION**, confirm at the P7 ops review |
| Ops burden | Backups ride Postgres PITR; realm export versioned in git; admin console restricted at Nginx; upgrades scheduled in pilot maintenance windows |
| Seam | The OIDC/OAuth2 boundary stays behind the provider-neutral adapter seam — swapping provider config is a config change, never code (DISPATCH D-06). Keycloak-specific surface stays inside `infra/keycloak/` + the adapter |
...
5. **Keycloak realm configuration** — Q8 resolved 2026-09-07 (Keycloak selected; SCAFFOLD §7). The OIDC adapter seam remains the portability investment: provider-specific code stays behind the adapter (DISPATCH D-06).
