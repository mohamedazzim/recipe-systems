...
**Rules honored:** no architecture invented (source-silent hops are labeled); Q1–Q7 and Q9–Q17 remain OPEN (Q8, Q18 resolved); Keycloak is the selected identity provider (Q8 resolved 2026-09-07); the workflow PNG is the existing asset, referenced as-is and never modified.
...
| IdP (Keycloak) | credential lifecycle, reset, email verification, OAuth/SSO, MFA — never application authorization | Tech Stack §14, ADR §19 |
...
| IdP internals | — | D-06 | Keycloak realm; no story (Tech Stack §14) |
...
| P1 (3–4) | D-06…D-09 | Keycloak IdP, cookies + account, guest/claim, ownership |
...
**Critical path: P0 → P1 → P2 → P3 → P4 → P5 → P6 → P7.** Track R feeds P4 and Track S feeds P5 but neither blocks the spine; P2 depends on P1's guest sessions only — IdP setup does not gate intake (BUILD_PLAN §2).
...
10. **Keycloak is the selected identity provider** (Q8 resolved 2026-09-07) — the OIDC adapter seam remains the portability investment; provider-specific code stays behind the adapter (BUILD_PLAN §5).

---

## 10. OPEN DECISIONS in scope (Q1–Q18 — Q8, Q18 resolved; none other resolved here)
...
| Q8 | Keycloak selected (2026-09-07) — sole identity provider | W1 step 1 (identity boundary) | RESOLVED |
...
| IdP internals / realm configuration | D-06 | no story; Keycloak realm (Tech Stack §14) |
