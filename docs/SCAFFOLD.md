...
- **Identity:** the IdP is **Keycloak (OIDC/OAuth2)** — the sole identity/authentication provider (**Q8 RESOLVED 2026-09-07**; Tech Stack §1/§14/§25/§26 amended to name Keycloak, superseding the earlier "Auth0 recommended" note). The IdP owns credential lifecycle, password reset, email verification, OAuth/SSO, and MFA — the list Tech Stack §14 assigns to it — and the application continues to own `account`, `guest_session`, recipe ownership, guest claiming, and authorization (ADR §19). The OIDC boundary is consumed through the provider-neutral adapter seam (BUILD_PLAN §5; DISPATCH D-06) — provider-specific code never escapes the adapter.
...
| Q2 | ~~Printed allergen-line source under the snapshot-only render rule~~ — **Resolved 2026-09-11 (Option A — analysis-time persistence):** the View 8 allergen line required on both prints is written into the print snapshot tables at analysis time (the station-card row for E5; the shopping-list generation snapshot for E4). Print/PDF consumes ONLY the persisted snapshot value and must NOT re-derive the allergen line at print time from the current effective-dated `dietary_allergen_mapping`. Rationale (dispatcher): analysis-time persistence is easier to audit; historical print output must remain stable; avoids future drift from effective-dated reference-table changes; avoids print-time re-derivation disagreement; the duplicated value is acceptable because it is a frozen print snapshot; INV-13 wording/safety constraints still apply. ADR §7 amended; IMPROVEMENT_PLAN P0-6 RESOLVED; CHANGE_LOG/HANDOFF 2026-09-11. | E4/H4/E5 vs ADR §7 permitted-source table |
...
| Q9 | ~~Final LLM provider after quality/cost/grounding benchmark~~ — **Resolved 2026-09-11 (dev provider):** DeepSeek integrated server-side behind the existing provider-neutral `LlmAdapter` seam (`packages/llm-adapter` `DeepSeekLlmAdapter`; worker `MODEL_PROVIDER=deepseek`, legacy `LLM_PROVIDER` accepted). Credentials verified outside Git; model `deepseek-v4-pro` validated against the account API (`GET /models`). The frozen D-05 schema gate, the D-16 grounding gate, regenerate-once, one-writer, and deterministic Views 8/9 are all UNCHANGED and re-proven against real model output (garlic absent-channel plant REJECTED; invalid output never published). CI/tests keep the deterministic mock (no key required). Evidence: HANDOFF §5 Q9 EXECUTION + CHANGE_LOG 2026-09-11. 2026-09-11 note: a second provider implementation (`GeminiLlmAdapter`, `gemini-3.8-flash`, `MODEL_PROVIDER=gemini`) was built, unit-tested and left AVAILABLE behind the same seam but is NOT selected — the one-recipe live switch was abandoned after provider-side 429 capacity windows (14 consecutive 429s; view 3 unfulfillable; decision recorded in CHANGE_LOG/HANDOFF). Runtime remains DeepSeek `deepseek-flash` + `reasoning_effort=low` (verified benchmark + live). | Tech Stack §1, §25.4 |
| Q10 | ~~Final OCR provider after real-card benchmarking~~ — **Resolved 2026-09-15 (DeepSeek Vision, `deepseek-flash`):** a second provider (`DeepSeekVisionOcrAdapter`, `OCR_PROVIDER=deepseek`) was added behind the provider-neutral seam and PASSED the canonical real handwritten golden card — 26/26 reference lines, 6/6 critical, 0 char-errors, garlic absent, both fenugreeks distinct, no fabrication, ~14.7 s latency; live browser photo→OCR→review→analysis completed with the DeepSeek worker. Confidence is provider-absent → the conservative `needs_review` policy flags every line (never an invented score). PaddleOCR stays available behind the seam but is NOT selected. **D-28 UNBLOCKED (not implemented).** Prior history preserved: 2026-09-09 attempts #1/#2 BLOCKED (no provenance-valid photos / credentials); 2026-09-15 PaddleOCR PP-OCRv3 EN FAILED the real card (7/26, 0/6 critical). Evidence: HANDOFF H-11 + AUDIT_LOG + CHANGE_LOG 2026-09-15. | Tech Stack §11, §25.5 |
