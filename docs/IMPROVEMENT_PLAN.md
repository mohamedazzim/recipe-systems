...
**Status:** v1.2 · 2026-09-07 · Gaps, risks, technical debt, and improvements, each traced to where the sources declare it. v1.1 review fix: priority-summary descriptions now match the sections. v1.2: Q8 resolved by reviewed patch (Keycloak selected 2026-09-07; see P1-1 below) — every other Q-item stays OPEN in the SCAFFOLD §7 register; resolving one requires the register's own process (a reviewed patch, never a silent edit).
...
| 🟠 P1 — gates later weeks | 5 | Q7, Q11, Q12, Q13, reviewer staffing (Q8 resolved 2026-09-07) |
...
**Options on the table:** run the harness on the D-04 corpus + real-card photos in weeks 1–4; record; decide. **Status:** ~~OPEN~~ → **RESOLVED 2026-09-15.** Q9 resolved 2026-09-11 (DeepSeek behind the `LlmAdapter` seam). Q10 resolved 2026-09-15 (DeepSeek Vision behind the OCR seam — `OCR_PROVIDER=deepseek` PASSED the canonical real handwritten golden card: 26/26 reference, 6/6 critical, 0 char-errors, garlic absent, both fenugreeks distinct, ~14.7 s latency; live browser photo→OCR→review→analysis completed; D-28 UNBLOCKED). Evidence: HANDOFF H-11 + CHANGE_LOG + AUDIT_LOG 2026-09-15.
...
### P1-1. Q8 — IdP: Keycloak *(resolved 2026-09-07)*

**Declared in:** Tech Stack §25.3 (the original recommendation was Auth0). **Gates:** P1 identity, but not intake (BUILD_PLAN §7.5 — P2 needs guest sessions only).
**Decision:** Keycloak is the sole identity/authentication provider (OIDC/OAuth2). Tech Stack §1/§14/§25/§26 amended to name Keycloak; ADR §19/§24 updated; Auth0 removed from the active architecture. All planning documents updated to match (SCAFFOLD §3/§7, BUILD_PLAN §5/§7, TEST_PLAN QG4/§4, DISPATCH D-06, AUDIT A-06, Epic A). **Status:** RESOLVED — supersedes the earlier working assumption (Keycloak pilot, Auth0 managed fallback).
**Re-run on resolution:** P1 gates per TEST_PLAN §5 when P1 starts — A-06 audit criteria now check decision fidelity (no alternate IdP), not Q8 hygiene.
...
- **The OIDC adapter seam is the portability investment** (Q8 resolved 2026-09-07): provider-specific code stays behind the adapter (DISPATCH D-06). Minimize Keycloak-specific surface outside `infra/keycloak/` + the adapter.
...
## Improvement backlog checklist (all OPEN except the resolved Q8 row)
...
- [x] Q8 answered (2026-09-07) → Keycloak selected; Tech Stack amended; P1 gates re-run when P1 starts
