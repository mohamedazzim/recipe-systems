...
**Date:** 2026-09-02  
**Amendment (2026-09-07):** the identity-provider recommendation is revised to **Keycloak (OIDC/OAuth2)** — the authoritative decision (OPEN DECISION Q8 resolved, SCAFFOLD §7). Sections §1, §14, §25, §26 updated accordingly; the original "Auth0 recommended" note is superseded and retained only as history (CHANGE_LOG.md).  
...
| Identity | Keycloak — OIDC/OAuth2 identity provider | Selected |
...
Keycloak is the sole identity/authentication provider for the pilot (decision recorded 2026-09-07 — OPEN DECISION Q8 resolved, SCAFFOLD §7). The OIDC/OAuth2 boundary is consumed through the provider-neutral adapter seam (BUILD_PLAN §5); application code never calls Keycloak-specific APIs outside that seam.
...
3.  Keycloak realm and required SSO providers.
