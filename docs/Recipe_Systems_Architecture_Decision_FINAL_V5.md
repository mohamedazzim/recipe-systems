...
**Status:** v1.4 — Proposed / ready for submission  
**Date:** 2026-09-02  
...
>
> **v1.4 revision note (2026-09-07):** §19 now names the selected identity provider — **Keycloak (OIDC/OAuth2)** — per the OPEN DECISION Q8 resolution recorded in SCAFFOLD §7 and the Tech Stack amendment of the same date. §24 item 6 updated to match. No other content changed.
...
**Amendment 2026-09-15 — F4 next-time line (D-26):** the station-card print additionally reads the LATEST `cook_log.next_time_instruction` for the owning recipe and surfaces it tagged **COOK LOG** (never CARD). This is post-cook data written after analysis completes, so it cannot ride the analysis-time station-card snapshot; the permitted-source table for the station card is therefore extended with `cook_log.next_time_instruction` (latest row, `cooked_at`/`created_at` DESC). The card's own CARD-tagged rows remain untouched — the next-time line is a distinct, provenance-tagged cook-log block. (DISPATCH D-26 deliverable 4; the F4 field itself is written by the API cook module via RS-US-34.)
...
Identity authentication is handled by **Keycloak**, the selected identity provider (OIDC/OAuth2 — Tech Stack §14, amended 2026-09-07; OPEN DECISION Q8 resolved, SCAFFOLD §7). The application owns application-specific authorization and recipe ownership.
...
6.  Keycloak realm and required SSO/MFA configuration.
