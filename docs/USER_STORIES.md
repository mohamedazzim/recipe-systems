...
**Status:** v1.2 · 2026-09-07 · The requirements source of truth for the 12-week pilot. This document **does not add requirements** — every line traces to the sources. v1.1 review fix: the could-haves (C7, E6, F5, I5) are labeled conditional per §13 and the traceability table names their dispatch unit (D-31). v1.2 normalization: the OPEN DECISION note updated for the Q8 resolution (Keycloak selected as the sole identity/authentication provider); no story ID, acceptance criterion, or test case changed.
...
- **OPEN DECISION note:** Q1–Q7 and Q9–Q17 in SCAFFOLD §7 remain OPEN (Q8 and Q18 are resolved — see SCAFFOLD §7); no story below silently resolves one. Where a story depends on an open decision, the DISPATCH unit carries the gate.
